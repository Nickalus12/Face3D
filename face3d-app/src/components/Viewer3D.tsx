...
export default function Viewer3D() {
  ...
  return (
    <div className={`viewer3d-container w-full h-full relative ${getBgClass(bgMode)}`}>
      {/* Floating Toolbar */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1 p-1 toolbar-glass rounded-xl">
        {/* View mode toggle */}
        <button
        ...
        {hasModel && modelInfo?.plyData && (
          <>
            <div className="bg-[#0a0a0b]/60 backdrop-blur-xl px-3 py-2 rounded-lg border border-white/5 shadow-2xl flex items-center justify-between gap-6 w-44">
              <span className="text-[11px] font-bold text-zinc-500 tracking-widest">
                POINTS
              </span>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1 p-1 toolbar-glass rounded-xl">
        {VIEW_PRESETS.map((preset) => {
          ...
          return (
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-400 hover:text-white hover:bg-white/5 transition-all duration-150 flex items-center gap-1.5 active:scale-95"
              title={preset.label}
            >
              <Icon size={13} />
              {preset.label}
      ...
      {/* Branding */}
      <div className="absolute bottom-14 left-4 z-10 pointer-events-none select-none">
        <div className="text-2xl font-black text-white/[0.03] tracking-tighter uppercase">
          Face3D Engine
        </div>
        <div className="text-[11px] font-bold text-indigo-400/30 uppercase tracking-[0.2em] mt-1 ml-1">
      ...
      {/* No model state — shows behind the 3D canvas with synthetic placeholder */}
      {noModel && (
        <div className="absolute inset-0 z-30 flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-5 max-w-sm text-center px-6 pointer-events-auto animate-fadeIn">
            <div className="w-20 h-20 rounded-2xl bg-zinc-800/30 border border-zinc-700/15 flex items-center justify-center backdrop-blur-sm">
            ...
            <div className="flex items-center gap-4 text-xs text-zinc-600">
              <span className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-emerald-500/40 ring-1 ring-emerald-500/20" />
                .ply point cloud
              </span>
              <span className="flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-blue-500/40 ring-1 ring-blue-500/20" />
                .obj mesh
              </span>
            </div>
          </div>
      ...
      >
        ...
        {/* Cinematic Particles — only when model is loaded */}
        {hasModel && (
          <Sparkles
            count={200}
            scale={8}
        ...
        {/* Axis Gizmo — only with model */}
        {hasModel && <GizmoHelper alignment="bottom-left" margin={[60, 60]}>
          <GizmoViewport
